...
export default function HomeScreen() {
  ...
  const { risk, weatherIsStale } = state;
  const hasData = true;
  const tierColor = getTierColor(theme, risk.tier);

  // Svg Geometry Parameters
  const strokeLength = 236;
  const gapLength = 59.31; 
  const dashArray = `${strokeLength} ${gapLength}`;
  
  // Set to 120.5 (the exact mathematical average between 118 and 123)
  const dashOffset = 120.5;
      ...
      <View style={styles.ring}>
        {hasData ? (
          <View style={styles.gaugeContainer}>
            ...
            {/* Top Right Tip: Rotated to exactly 19.75° to hit the balanced track center */}
            <View style={[styles.gaugeRotator, { transform: [{ rotate: '19.75deg' }] }]}>
              <View style={styles.topIconAnchor}>
                <Icon name="bad-weather" size={16} color={theme.mode === 'dark' ? '#000000' : '#FFFFFF'}  />
              </View>
            </View>

            {/* Bottom Left Tip: Rotated to exactly 19.75° to hit the balanced track center */}
            <View style={[styles.gaugeRotator, { transform: [{ rotate: '19.75deg' }] }]}>
              <View style={styles.bottomIconAnchor}>
                <Icon name="scheduled-maintenance" size={16} color={theme.mode === 'dark' ? '#000000' : '#FFFFFF'}  />
              </View>
            </View>
            ...
            <View style={styles.gaugeCenterLabels}>
              ...
              <Text style={styles.riskSubtext}>next 2 hours</Text>
            </View>
          </View>
        ) : (
...
function makeStyles(theme: OhmTheme) {
  return StyleSheet.create({
    content: { padding: 20, paddingTop: 30, paddingBottom: 100, flexGrow: 1 },
    ...
    riskTierText: { fontSize: 20, fontWeight: '900', fontFamily: 'Gilroy-Black' },
    ...
    gaugeRotator: { position: 'absolute', width: 260, height: 260, alignItems: 'center' },
    
    // Top-Right Tip Anchor
    topIconAnchor: { marginTop: 25, width: 24, height: 24, alignItems: 'center', justifyContent: 'center', zIndex: 5 },
    
    // Bottom-Left Tip Anchor
    bottomIconAnchor: { marginTop: 211, width: 24, height: 24, alignItems: 'center', justifyContent: 'center', zIndex: 5 },
  });
}
